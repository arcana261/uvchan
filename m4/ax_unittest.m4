...
[
    ...
    AM_CONDITIONAL([HAVE_UNITTEST], [test x$XARGS != x -a x$HAS_SETJMP = xtrue -a x$ax_pthread_ok = xyes -a x$HAS_SCHED = xtrue -a x$HAS_SYS_TIME = xtrue -a x$HAS_UNISTD = xtrue -a x$HAS_SIGNAL = xtrue -a x$HAS_ERRNO = xtrue])
